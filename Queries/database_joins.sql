...
-- Joining retirement_info & dept_emp tables
...
-- JOIN retirement_info & dept_emp tables into new table 'current_emp'
SELECT ri.emp_no,
	ri.first_name,
	ri.last_name,
	de.to_date
...
-- Employee count by deptartment number sorted
SELECT COUNT(ce.emp_no), de.dept_no
FROM current_emp as ce
LEFT JOIN dept_emp as de ON ce.emp_no = de.emp_no
GROUP BY de.dept_no
...
-- Add emp_no, first & last name, gender, salary, and employment status to new table
SELECT e.emp_no,
	e.first_name,
	e.last_name,
	e.gender,
...
-- List 2) Management
	-- Manager emp_no, names, starting date, end date
...
INNER JOIN departments as d ON (de.dept_no = d.dept_no);postgres
